...
def GetEcardBills(session:requests.Session, _QueryDate:str|list[str]|tuple[str,str]) -> dict:
    ...
    if isinstance(_QueryDate, list):
        _QueryDate = [DateFormat(query_date) for query_date in _QueryDate] # 格式化
        ...
        for _QueryMonth in month_list:
            ret.extend([entry
                for entry in __GetMonth(session,account,_QueryMonth)
                if entry['time'][:10] in _QueryDate])

    elif isinstance(_QueryDate, tuple):
        _QueryDate = ( # 格式化日期区间
            DateFormat(_QueryDate[0]),
            DateFormat(_QueryDate[1])
        )
        ret = []
        month_list = __get_yyyy_mm_between(_QueryDate)
        for _QueryMonth in month_list:
            if _QueryMonth is month_list[0] or _QueryMonth is month_list[-1]:
                ret.extend([entry  # 头尾月份检测是否在日期区间内
                           for entry in __GetMonth(session,account,_QueryMonth)
                           if __is_inbetween(entry['time'][:10],_QueryDate)])
            else:
                ret.extend(__GetMonth(session,account,_QueryMonth))
        ...
        if _QueryDate.count('-') == 1: 
            ...
            return __GetMonth(session,account,_QueryDate) # 直接返回当月数据
        # Date
        elif _QueryDate.count('-') == 2:
            _QueryDate = DateFormat(_QueryDate) # 格式化日期
            return [entry 
                    for entry in __GetMonth(session,account,_QueryDate) 
...
def __GetMonth(session:requests.Session, account:str, _QueryMonth:str) -> list:
    '''
    PARAMETERS:\n
    session     -- should be already logged in\n
...
def __get_yyyy_mm_between(date_range:tuple) -> list:
